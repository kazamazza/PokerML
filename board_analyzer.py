...
class BoardAnalyzer:
    ...
    def analyze(self, board: List[str]) -> BoardTexture:
        ...
        is_monotone = any(count == len(board) for count in suit_counts.values())
        is_two_tone = list(suit_counts.values()).count(2) == 1
        suit_texture = "monotone" if is_monotone else "two-tone" if is_two_tone else "rainbow"
